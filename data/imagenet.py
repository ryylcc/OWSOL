...
from config import imagenet_root, imagenet_meta
...
def get_imagenet_datasets(train_transform, test_transform):

    meta_train = os.path.join(imagenet_meta, 'train')
    meta_val = os.path.join(imagenet_meta, 'val')
    meta_test = os.path.join(imagenet_meta, 'test')

    train_dataset = TrainDataset(imagenet_root, meta_train, train_transform)
    cluster_dataset = MclDataset(imagenet_root, meta_train, test_transform)
    val_dataset = EvalDataset(imagenet_root, meta_val, test_transform)

    # return train_dataset, test_dataset, val_dataset
    return train_dataset, cluster_dataset, val_dataset


def get_imagenet_datasets_cluster(test_transform):

    meta_test = os.path.join(imagenet_meta, 'test')
    test_dataset = ClusterDataset(imagenet_root, meta_test, test_transform)

    return test_dataset


def get_imagenet_datasets_gcam(test_transform, target_and_pred):

    meta_test = os.path.join(imagenet_meta, 'test')
    test_dataset = GcamDataSet(imagenet_root, meta_test, target_and_pred, test_transform)

    return test_dataset

def get_imagenet_datasets_estimate_k(test_transform):

    meta_val = os.path.join(imagenet_meta, 'val')
    val_dataset = ClusterDataset(imagenet_root, meta_val, test_transform)
...
if __name__=='__main__':
    ...
    dataset = TrainDataset('/data/zhaochuan/NCL/code/wsolevaluation-master/dataset/ILSVRC/', '/data/zhaochuan/NCL/code/GWSOL/metadata/ImageNet/train/', transforms.Compose(augmentation))
