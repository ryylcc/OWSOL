...
from config import inatloc_root, inatloc_meta
...
def get_inatloc_datasets(train_transform, test_transform):

    meta_train = os.path.join(inatloc_meta, 'train')
    meta_val = os.path.join(inatloc_meta, 'val')
    meta_test = os.path.join(inatloc_meta, 'test')

    train_dataset = TrainDataset(inatloc_root, meta_train, train_transform)
    cluster_dataset = MclDataset(inatloc_root, meta_train, test_transform)
    val_dataset = EvalDataset(inatloc_root, meta_val, test_transform)

    # return train_dataset, test_dataset, val_dataset
    return train_dataset, cluster_dataset, val_dataset


def get_inatloc_datasets_cluster(test_transform):

    meta_test = os.path.join(inatloc_meta, 'test')
    test_dataset = ClusterDataset(inatloc_root, meta_test, test_transform)

    return test_dataset


def get_inatloc_datasets_gcam(test_transform, target_and_pred):

    meta_test = os.path.join(inatloc_meta, 'test')
    test_dataset = GcamDataSet(inatloc_root, meta_test, target_and_pred, test_transform)

    return test_dataset

def get_inatloc_datasets_estimate_k(test_transform):

    val_test = os.path.join(inatloc_meta, 'val')
    val_dataset = ClusterDataset(inatloc_root, val_test, test_transform)
...
if __name__=='__main__':
    ...
    dataset = TrainDataset('/data/zhaochuan/NCL/code/wsolevaluation-master/dataset/ILSVRC/', '/data/zhaochuan/NCL/code/GWSOL/metadata/inatloc/train/', transforms.Compose(augmentation))
